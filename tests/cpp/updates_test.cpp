...
int main(int argc, char **argv) {
    int M = 16;                                             //* Max edge of each node
    int efConstruction = 200;                               //* num of candidatenode
    int num_threads = std::thread::hardware_concurrency();  //* get current CPU's thread number

    bool update = false;

    if (argc == 2) {                                        //* command arg processing
    ...
    std::string path = "../tests/cpp/data/";                //* path of data for the test (Generated by update_gen_data.py)
                                                            //* Test config
    int N;                                                  //* - datasize
    int dummy_data_multiplier;                              //* - num of batch
    int N_queries;                                          //* - num of queries
    ...
    hnswlib::L2Space l2space(d);                            //* init L2 dist space
    hnswlib::HierarchicalNSW<float> appr_alg(&l2space, N + 1, M, efConstruction);   //* init HNSW graph object

    std::vector<float> dummy_batch = load_batch<float>(path + "batch_dummy_00.bin", N * d); //* load N-node from batchfile

    // Adding enterpoint:

    appr_alg.addPoint((void *)dummy_batch.data(), (size_t)0);               //* add node_0 for graph init

    StopW stopw = StopW();

    if (update) {                                                           //* not for the real graph construction, but just for checking integrity of graph construction algorithm
        std::cout << "Update iteration 0\n";

        ParallelFor(1, N, num_threads, [&](size_t i, size_t threadId) {     //* Multi-threaded for-loop (node_1~N-1)
            appr_alg.addPoint((void *)(dummy_batch.data() + i * d), i);     //* add nodes to HNSW graph 
        });
        appr_alg.checkIntegrity();                                          //* check graph integrity

        ParallelFor(1, N, num_threads, [&](size_t i, size_t threadId) {     //* one more time with same data to check integrity
            appr_alg.addPoint((void *)(dummy_batch.data() + i * d), i);
        });
        appr_alg.checkIntegrity();

        for (int b = 1; b < dummy_data_multiplier; b++) {                   //* multi-batch for-loop (only if dummy_data_multiplier > 1)
